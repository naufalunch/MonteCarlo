...
if uploaded_file is not None:
    ...
    for i in range(len(y)):   
        res += y[i] / frek
        st.sidebar.text("Probabilitas Komulatif "+str(i)+": "+str(round(res,3)))
        reslist.append(res)
    st.sidebar.text("")  
    ...
    des = 229900
    st.text("Harga motor: Rp."+str(rupiah_format(des)))
